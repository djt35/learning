		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <div class='row'>
		                ...
		                <div id="messageBox" class='col-3 yellow-light narrow center'>
		                    <p><button id="newvalues" onclick="window.location.href = '<?php echo BASE_URL;?>/scripts/forms/valuesForm.php';">New values</button></p>
		                </div>
					</div>
			        ...
			        <div class='row'>
		                ...
		                <div class='col-10 narrow' style='overflow-x: scroll;'>
		                    <p><?php $general->makeSearchableTableDelete("SELECT `id`, `Yes_No`, `Yes_No_t`, `specialistInterest`, `specialistInterest_t`, `emailPreferences`, `emailPreferences_t`, `access_level`, `access_level_t` from `values`"); ?></p>
		                </div>
		...
		var tagsid = null;
			...
			$(document).ready(function() {
				...
				$('.content').on('click', '.deleteTag', function () {
					...
					if (confirm('Do you wish to delete this values [can\'t be undone]?')) {

						var id = $(this).closest('tr').find('td:eq(0)').text();

						var tr = $(this).closest('tr');

						var imagesObject = pushDataAJAX('values', 'id', id, 2, ''); //delete values

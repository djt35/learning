		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <div class='row'>
		                ...
		                <div id="messageBox" class='col-3 yellow-light narrow center'>
		                    <p><button id="newusers" onclick="window.location.href = '<?php echo BASE_URL;?>/scripts/forms/usersForm.php';">New users</button></p>
		                </div>
					</div>
			        ...
			        <div class='row'>
		                ...
		                <div class='col-10 narrow' style='overflow-x: scroll;'>
		                    <p><?php $general->makeSearchableTableDelete("SELECT `user_id`, `firstname`, `surname`, `email`, `password`, `centre`, `centreName`, `registered_date`, `last_login`, `previous_login`, `timezone`, `access_level`, `contactPhone`, `key`, `centreCity`, `centreCountry`, `trainee`, `yearsIndependent`, `endoscopyTrainingProgramme`, `yearsEndoscopy`, `specialistInterest`, `emailPreferences` from `users`"); ?></p>
		                </div>
		...
		var tagsid = null;
			...
			$(document).ready(function() {
				...
				$('.content').on('click', '.deleteTag', function () {
					...
					if (confirm('Do you wish to delete this users [can\'t be undone]?')) {

						var id = $(this).closest('tr').find('td:eq(0)').text();

						var tr = $(this).closest('tr');

						var imagesObject = pushDataAJAX('users', 'id', id, 2, ''); //delete users

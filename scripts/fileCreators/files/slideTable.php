		...
		<body>
		    ...
		    <div id='content' class='content'>
		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <div class='row'>
		                <div class='col-9'>
		                    <h2 style="text-align:left;">List of slide</h2>
		                </div>
		
		                <div id="messageBox" class='col-3 yellow-light narrow center'>
		                    <p><button id="newslide" onclick="window.location.href = '<?php echo BASE_URL;?>/scripts/forms/slideForm.php';">New slide</button></p>
		                </div>
					</div>
			        ...
			        <div class='row'>
		                ...
		                <div class='col-10 narrow' style='overflow-x: scroll;'>
		                    <p><?php $general->makeSearchableTableDelete("SELECT `id`, `audio_id`, `video_id` from `slide`"); ?></p>
		                </div>
		...
		var tagsid = null;
			...
			$(document).ready(function() {
				...
				$(".content").on("click", ".datarow", function(){
					...
					window.location.href = siteRoot + 'scripts/forms/slideForm.php?id=' + id;
				...
				$('.content').on('click', '.deleteTag', function () {
					...
					if (confirm('Do you wish to delete this slide [can\'t be undone]?')) {

						var id = $(this).closest('tr').find('td:eq(0)').text();

						var tr = $(this).closest('tr');

						var imagesObject = pushDataAJAX('slide', 'id', id, 2, ''); //delete slide

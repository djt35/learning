		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <div class='row'>
		                ...
		                <div id="messageBox" class='col-3 yellow-light narrow center'>
		                    <p><button id="newvideo" onclick="window.location.href = '<?php echo BASE_URL;?>/scripts/forms/videoForm.php';">New video</button></p>
		                </div>
					</div>
			        ...
			        <div class='row'>
		                ...
		                <div class='col-10 narrow' style='overflow-x: scroll;'>
		                    <p><?php $general->makeSearchableTableDelete("SELECT `id`, `name`, `url`, `active`, `split`, `created`, `updated`, `author`, `description`, `duration`, `thumbnail`, `paid` from `video`"); ?></p>
		                </div>
		...
		var tagsid = null;
			...
			$(document).ready(function() {
				...
				$('.content').on('click', '.deleteTag', function () {
					...
					if (confirm('Do you wish to delete this video [can\'t be undone]?')) {

						var id = $(this).closest('tr').find('td:eq(0)').text();

						var tr = $(this).closest('tr');

						var imagesObject = pushDataAJAX('video', 'id', id, 2, ''); //delete video

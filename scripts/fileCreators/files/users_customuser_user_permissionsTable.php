		...
		<html>
		<head>
		    <title>users_customuser_user_permissions Table</title>
		</head>
		...
		<body>
		    ...
		    <div id='content' class='content'>
		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <div class='row'>
		                <div class='col-9'>
		                    <h2 style="text-align:left;">List of users_customuser_user_permissions</h2>
		                </div>
		
		                <div id="messageBox" class='col-3 yellow-light narrow center'>
		                    <p><button id="newusers_customuser_user_permissions" onclick="window.location.href = '<?php echo BASE_URL;?>/scripts/forms/users_customuser_user_permissionsForm.php';">New users_customuser_user_permissions</button></p>
		                </div>
					</div>
			        ...
			        <div class='row'>
		                ...
		                <div class='col-10 narrow' style='overflow-x: scroll;'>
		                    <p><?php $general->makeSearchableTableDelete("SELECT `id`, `customuser_id`, `permission_id` from `users_customuser_user_permissions`"); ?></p>
		                </div>
		...
		var tagsid = null;
			...
			$(document).ready(function() {
				...
				$(".content").on("click", ".datarow", function(){
					...
					window.location.href = siteRoot + 'scripts/forms/users_customuser_user_permissionsForm.php?id=' + id;
				...
				$('.content').on('click', '.deleteTag', function () {
					...
					if (confirm('Do you wish to delete this users_customuser_user_permissions [can\'t be undone]?')) {

						var id = $(this).closest('tr').find('td:eq(0)').text();

						var tr = $(this).closest('tr');

						var imagesObject = pushDataAJAX('users_customuser_user_permissions', 'id', id, 2, ''); //delete users_customuser_user_permissions

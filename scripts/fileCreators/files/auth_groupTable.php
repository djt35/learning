		...
		<body>
		    ...
		    <div id='content' class='content'>
		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <div class='row'>
		                <div class='col-9'>
		                    <h2 style="text-align:left;">List of auth_group</h2>
		                </div>
		
		                <div id="messageBox" class='col-3 yellow-light narrow center'>
		                    <p><button id="newauth_group" onclick="window.location.href = '<?php echo BASE_URL;?>/scripts/forms/auth_groupForm.php';">New auth_group</button></p>
		                </div>
					</div>
			        ...
			        <div class='row'>
		                ...
		                <div class='col-10 narrow' style='overflow-x: scroll;'>
		                    <p><?php $general->makeSearchableTableDelete("SELECT `id`, `name` from `auth_group`"); ?></p>
		                </div>
		...
		var tagsid = null;
			...
			$(document).ready(function() {
				...
				$(".content").on("click", ".datarow", function(){
					...
					window.location.href = siteRoot + 'scripts/forms/auth_groupForm.php?id=' + id;
				...
				$('.content').on('click', '.deleteTag', function () {
					...
					if (confirm('Do you wish to delete this auth_group [can\'t be undone]?')) {

						var id = $(this).closest('tr').find('td:eq(0)').text();

						var tr = $(this).closest('tr');

						var imagesObject = pushDataAJAX('auth_group', 'id', id, 2, ''); //delete auth_group

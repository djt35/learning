		...
		<html>
		<head>
		    <title>userLearningToolAccess Table</title>
		</head>
		...
		<body>
		    ...
		    <div id='content' class='content'>
		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <div class='row'>
		                <div class='col-9'>
		                    <h2 style="text-align:left;">List of userLearningToolAccess</h2>
		                </div>
		
		                <div id="messageBox" class='col-3 yellow-light narrow center'>
		                    <p><button id="newuserLearningToolAccess" onclick="window.location.href = '<?php echo BASE_URL;?>/scripts/forms/userLearningToolAccessForm.php';">New userLearningToolAccess</button></p>
		                </div>
					</div>
			        ...
			        <div class='row'>
		                ...
		                <div class='col-10 narrow' style='overflow-x: scroll;'>
		                    <p><?php $general->makeSearchableTableDelete("SELECT `id`, `userid`, `learningToolid`, `access` from `userLearningToolAccess`"); ?></p>
		                </div>
		...
		var tagsid = null;
			...
			$(document).ready(function() {
				...
				$(".content").on("click", ".datarow", function(){
					...
					window.location.href = siteRoot + 'scripts/forms/userLearningToolAccessForm.php?id=' + id;
				...
				$('.content').on('click', '.deleteTag', function () {
					...
					if (confirm('Do you wish to delete this userLearningToolAccess [can\'t be undone]?')) {

						var id = $(this).closest('tr').find('td:eq(0)').text();

						var tr = $(this).closest('tr');

						var imagesObject = pushDataAJAX('userLearningToolAccess', 'id', id, 2, ''); //delete userLearningToolAccess

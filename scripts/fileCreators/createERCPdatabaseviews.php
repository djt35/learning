	...
	foreach ($datafields as $key=>$value){
		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <p><?php
				        ...
				        if (\$id){
							...
							\$q = \"SELECT  {$databaseIdentifier}  FROM  ESD.{$databaseTable}  WHERE  {$databaseIdentifier}  = \$id\";
		...
		<script>
			...
			function fillForm (idPassed){
				...
				if($(\"form#{$databaseTable}\").find(\"button#delete{$databaseTable}\").length == 0){

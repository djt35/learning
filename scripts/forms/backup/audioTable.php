		        ...
		        <div class='responsiveContainer white'>
			        ...
			        <div class='row'>
		                ...
		                <div id="messageBox" class='col-3 yellow-light narrow center'>
		                    <p><button id="newaudio" onclick="window.location.href = '<?php echo BASE_URL;?>/scripts/forms/audioForm.php';">New audio</button></p>
		                </div>
					</div>
			        ...
			        <div class='row'>
		                ...
		                <div class='col-10 narrow' style='overflow-x: scroll;'>
		                    <p><?php $general->makeSearchableTableDelete("SELECT `id`, `name`, `url` from `audio`"); ?></p>
		                </div>
		...
		var tagsid = null;
			...
			$(document).ready(function() {
				...
				$('.content').on('click', '.deleteTag', function () {
					...
					if (confirm('Do you wish to delete this audio [can\'t be undone]?')) {

						var id = $(this).closest('tr').find('td:eq(0)').text();

						var tr = $(this).closest('tr');

						var imagesObject = pushDataAJAX('audio', 'id', id, 2, ''); //delete audio

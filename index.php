...
<body>
			...
			<div class='row'>
				<div class='col-9'>
					<h2 class='' id="pageTitle" style="text-align:left; margin:0;">
						<?php echo 'Quality photos, videos and learning in Endoscopy and Endoscopic Resection';?></h2>
				</div>
					...
					<div class='row'>
						<div class='col-12 yellow-light center'>
							<div id='chapterSelector'>
								Welcome to endoscopy<sub>wiki</sub>
								<?php ?>
							</div>
					...
					<div class='row'>
						...
						<div class='col-12 black whiteborder center news'>
							...
							<div id='chapterInfo'>
								...
								<p id='chapterHeading'><b>New cases on endoscopy<sub>wiki</sub></b></p>

								<?php $blogs = $general->getBlogNewsPosts(); //print_r($blogs);?>
								<div class='responsiveContainer'>
								<?php
									...
									foreach ($blogs as $key=>$value){
										...
										/*echo "<div class='row'>";
										echo "<div class='col-12 black left news'>";
										echo "</div>";
										echo "</div>";*/

										echo "<div class='row hover' data='{$value['id']}'>";
										echo "<div class='col-5 black small center news'>";
										echo "<img src='" . BASE_URL . "/{$value['url']}' style='max-width:100%;'>";
										echo "</div>";

										echo "<div class='col-7 black center news' style='font-size: 12px;'>";
										echo $value['title'] . '<br><br>' . 'Author: ' . $value['author'] . '<br>' . 'Posted: ' . $value['dateCreated'];
										echo "</div>";
										echo "</div>";
									}
    ...
    */
				...
				manipulation.done(function (data) {
					...
					if (data) {
						...
						$('.modal').find('.modalContent').append(
							'<p>Thanks for expressing your interest in Endoscopy Wiki. <br> Joining is free. <br> Multiple high quality learning opportunities are only a few clicks away.</p>'
							);
						...
						$("#users").validate({
							...
							messages: {
								...
								centreCity: {
									required: 'Please enter your institution city'
								},
								specialistInterest: {
									required: 'Please select your specialist interest'
								},
								trainee: {
									required: 'Are you a trainee?'
								},
								yearsIndependent: {
									required: 'How many years have you been practising your specialty (incl. training)'
								},
								yearsEndoscopy: {
									required: 'How many years have you been performing endoscopy?'
								},
								endoscopyTrainingProgramme: {
									required: 'Required'
								},
			...
			$('.content').on("mouseenter", '.hover', function (){


				$(this).children().css("background-color", "#205690").css("cursor", "pointer");


			})

			$('.content').on("click", '.hover', function (){

				var target = $(this).attr("data");
				window.location = siteRoot+'/scripts/display/casePost.php?id='+target;
			...
			$('.content').on("mouseleave", '.hover', function (){


				$(this).children().css("background-color", "black").css("cursor", "default");
			...
			$(".modal").on("click", "#forgot", function () {
				...
				manipulation.done(function (data) {
					...
					if (data) {
						...
						$('.modal').find('.modalContent').append(
							'<p>Enter your email address below. <br> If this email address is registered you will receive a password reset email.</p><br>'
							);

						$('.modal').find('.modalContent').append(data);

						$('.modal').find('.modalContent').append(
							'<div class="errorTxt" style="font-size:12px;"></div>');
